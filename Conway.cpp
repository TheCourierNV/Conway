...
void Conway::run() {
    ...
    // Windows and Linux represent the arrow keys using a series of different
    // values. Here we define the exact values to expect in each platform
...
#define OS_ARROW_RIGHT 67
    ...
    do {
        ...
        if (firstChar != OS_ARROW_INIT) {
            ...
            // If firstChar doesn't match any 'special key', we stop waiting for
...
#ifdef ISWINDOWS
        (void)extraLinuxChar; // Since this variable is completely unused on
                              // Windows, we do this to avoid triggering "unused
                              // variable" warnings
#endif
        ...
        if (lastKey == OS_ARROW_UP && m_selectedOffsetY > 0) { // Pressed "up"
            m_selectedOffsetY--;
        } else if (lastKey == OS_ARROW_DOWN &&
                   m_selectedOffsetY <
                       m_Board.getSizeY() - 1) { // Pressed "down"
            m_selectedOffsetY++;
        } else if (lastKey == OS_ARROW_RIGHT &&
                   m_selectedOffsetX <
                       m_Board.getSizeX() - 1) { // Pressed "right"
            m_selectedOffsetX++;
        } else if (lastKey == OS_ARROW_LEFT &&
                   m_selectedOffsetX > 0) { // Pressed "down"
            m_selectedOffsetX--;
        }
    ...
    while (true) {
        ...
        sleepFor(m_minimumFrameDelta); // We pause execution for a few
                                       // milliseconds to prevent flickering.
    }
}
...
void Conway::printNewBoard() {
    ...
    for (unsigned int y = 0; y < m_Board.getSizeY(); y++) {
        for (unsigned int x = 0; x < m_Board.getSizeX(); x++) {
            ...
            // A living cell is represented by a green 'O'. while a dead cell is
        ...
        for (int y = 0; y < sizeY; y++) {
            ...
            for (int offset1 = -1; offset1 <= 1; offset1++) {
                for (int offset2 = -1; offset2 <= 1; offset2++) {
                    ...
                    bool isOffsetNotInBounds =
                        (offset1 == 0 && offset2 == 0) || (totalOff1 < 0) ||
                        (totalOff2 < 0) ||
                        ((unsigned)totalOff1 >= m_Board.getSizeY()) ||
                        (unsigned)totalOff2 >= m_Board.getSizeX();
            ...
            case 2:
                tmpCopy[y][x] =
                    m_Board[y][x].isAlive; // Keep the previous state
                break;
            default:
                tmpCopy[y][x] = false;
                break;
