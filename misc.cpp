...
// Function that prints text in a given Color. The actual implementation differs massively depending on the operating system.
void printInColor(const char* toPrint, Color color){
    // Windows version:
        ...
        // Get the exact console settings (including color) we were using before printing so that we can restore them after we are done.
    ...
    //Linux version:
    // In Linux changing the color of text in a console is very easy: you just need to print an ANSI color code beforehand.
    #elif __linux__
        ...
        std::cout << unixColorString << toPrint << "\033[0m"; // Print the desired string and reset the color afterwards
    #endif
...
// Windows and linux use slightly different commands to clear the screen, so we use a wrapper function
void clearScreen(){
    std::string command;
...
// Print text in a way similar to the "less" command on Unix systems: one "page" at a time
