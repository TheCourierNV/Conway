...
template <typename T>
myMatrix<T>::myMatrix(unsigned int dimensionXY)
    : myMatrix(dimensionXY, dimensionXY){};
...
template <typename T>
void myMatrix<T>::allocateMatrix(unsigned int dimensionX,
...
template <typename T> unsigned int myMatrix<T>::getSizeX() { return m_rowSize; }

// Getter per la dimensione Y
template <typename T> unsigned int myMatrix<T>::getSizeY() {
    return m_rowAmount;
}

template <typename T> void myMatrix<T>::matrixRow::allocate(unsigned int size) {
    m_array = new T[m_size];
}

template <typename T> myMatrix<T>::matrixRow::matrixRow() { m_size = 0; }
